...
export function Marketplace({
  ...
  // Vault cards owned by user that are NOT currently listed as active
  const activeListingCardIds = new Set(myListings.filter(l => l.status === 'active').map(l => l.cardId));
  const availableVaultCards = allCards.filter(c => vaultIds.has(c.id) && !activeListingCardIds.has(c.id));
  ...
  const handleBuyMarketCard = async (listing: MarketListing) => {
    ...
    try {
      ...
      // 1. Deduct ARTCOIN from buyer & add card to buyer's vault
      const nextBuyerVault = new Set(vaultIds);
      nextBuyerVault.add(listing.cardId);

      await setDoc(buyerRef, {
        email: user.email,
        walletBalance: increment(-listing.price),
        vaultIds: Array.from(nextBuyerVault),
        collectionIds: Array.from(nextBuyerVault)
      }, { merge: true });

      // 2. Credit ARTCOIN to seller & remove card from seller's vault
      // Fetch current seller doc to update their vault
      const sellerSnap = await doc(db, 'users', listing.sellerId);
      await updateDoc(sellerSnap, {
        walletBalance: increment(listing.price)
      }).catch(async () => {
        await setDoc(sellerSnap, {
          walletBalance: increment(listing.price)
        }, { merge: true });
      });
